...
> 한국 古典·국어학 PDF에서 깨진 글자(구결자·옛한글)를 표준 Unicode로 자동 복원하는 Python CLI.

`pdfplumber` 같은 일반 도구로 한국 학술 PDF를 추출하면 `(cid:6)`처럼 깨지는 글자가 두 종류:

- **구결자(口訣字)** — 漢文 옆에 토를 적은 약식 글자 (匕→니, 古→고, 兯→한 등)
- **옛한글** — 아래아·반치음 등이 들어간 자모 (ᄒᆞ, ᇫ 등)

본 도구는 표준 매핑 데이터를 활용해 이들을 정상 Unicode로 복원하고, PDF 본문 전체를 깨끗한 markdown으로 출력합니다.
...
python scripts/decode.py <PDF경로>          # PDF → <PDF>.normalized.md
```
...
- **1-step CLI**: `decode.py <PDF>` 한 번으로 추출 + 매핑 + 정규화
- **자동 매핑**: hypua + AKS 표준 매핑 데이터로 시각 판독 거의 불필요
- **합자 구결 스캔**: 표준 Unicode CJK 영역의 합자(兯 등)도 옵션 처리
